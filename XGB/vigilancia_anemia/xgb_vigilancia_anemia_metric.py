...
# BASE ESTIMATOR
clf = xgb.XGBClassifier(objective='binary:logistic', booster='gbtree', random_state=RANDOM_STATE, n_jobs=-1)
# INNER CV: GRID SEARCH
grid_search = GridSearchCV(clf, cv=rskf, param_grid=PARAMS, scoring=SCORING, n_jobs=-1)
# OUTER CV: cross_val_predict
preds = cross_val_predict(grid_search, X, y, cv=skf, method='predict_proba', n_jobs=-1)
